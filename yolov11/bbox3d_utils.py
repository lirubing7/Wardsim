...
class BBox3DEstimator:
    ...
    def draw_box_3d(self, image, box_3d, color=(0, 255, 0), thickness=2):
        ...
        """
        # 新增：检测到 obj_id=None，直接返回不绘制 3D 框
        obj_id = box_3d.get('object_id')
        if obj_id is None:
            return image  # 不生成 3D 框
...
class BirdEyeView:
    ...
    def reset(self):
        ...
        for dist in [1, 1.5, 2, 2.5, 3, 3.5, 4, 4.5, 5]:
            ...
            thickness = 2 if isinstance(dist, float) else 1
            cv2.line(self.bev_image, 
                    (self.origin_x - 5, y), 
                    (self.origin_x + 5, y), 
    ...
    def draw_box(self, box_3d, color=None):
        ...
        try:
            ...
            #depth_value = float(box_3d['depth_value'])  # 深度值转为浮点数
            ...
            bbox_2d = list(map(float, box_3d['bbox_2d']))  # 2D坐标转为浮点数（xmin,ymin,xmax,ymax）
